...
michelin <- read_csv("michelin.csv") %>% ##carrega os dados
    separate(Award, c("star", "star_type"), sep = ",") %>% ##separa a variável em duas
  separate(Location, c("city", "country"), sep = ", ")


dados_rest_inicial <- michelin %>% 
  filter(Name == "Gabelspiel") ## restaurante inicial
...
```{r}
michelin %>% 
  filter(star == "1 Star" & city != dados_rest_inicial$city) %>% 
...
michelin %>% 
  filter(star %in% vetor & dist_rest_inicial <= 3000) %>% 
  nrow()
```
...
```{r}
michelin %>% 
  filter(dist_rest_inicial <= 2000, star %in% vetor, str_length(Price) <= 2, 
        city != dados_rest_inicial$city) %>% ## verifica todas as condições
  nrow()
...
michelin %>% 
  ...
  filter((str_detect(cuisine_1, "^Country cooking$")| str_detect(cuisine_2, "^Country cooking$")) & city != dados_rest_inicial$city) %>% ## filtra por padrão de caracteres
...
```{r}
dr_who %>% 
  filter(director == "Rachel Talalay", writer == "Steven Moffat") %>% 
...
dr_who %>% 
  filter(writer == "Russell T Davies", ano == 2005) %>% 
